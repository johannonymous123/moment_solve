...
# ============================================================
# 7) Main DG solver (lagged D)
#    FIX: higher-order face coupling via trace + lift (correct DG structure)
# ============================================================
    ...
    for it in range(max_picard):
        ...
        for iy in range(ny):
            for ix in range(nx):
                ...
                # phi:  sa M *phi  - Gx*Jx - Gy*Jy = M*Q
                # Jx : -(Dxx Gx + Dxy Gy)*phi + st M*Jx = 0
                # Jy : -(Dyx Gx + Dyy Gy)*phi + st M*Jy = 0
                ...
                # row/col blocks by component offsets
                off_phi = off + 0 * Nloc
                off_Jx  = off + 1 * Nloc
                off_Jy  = off + 2 * Nloc
...
# ============================================================
# 8) Example usage (plots kept as-is; material functions unchanged)
# ============================================================
...
if __name__ == "__main__":
    ...
    Jx_avg = sol.Jx[:, 0]
    Jy_avg = sol.Jy[:, 0]
    print("phi_avg min/max:", phi_avg.min(), phi_avg.max())

    import matplotlib.pyplot as plt

    nx, ny = grid.nx, grid.ny

    phi_img = phi_avg.reshape((ny, nx))
